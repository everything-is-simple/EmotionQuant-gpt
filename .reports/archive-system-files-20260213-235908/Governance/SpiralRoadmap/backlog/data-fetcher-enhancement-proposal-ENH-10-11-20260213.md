...
**提案日期**: 2026-02-13  
**提案人**: 用户需求  
**状态**: Accepted（已纳入执行路线，2026-02-13）  
...
| 编号 | 名称 | 目标 | 实施圈 | 预估工时 |
|---|---|---|---|---|
| ENH-10 | 数据采集增强 | 分批+断点续传+多线程 | S3a（S2后、S3前） | 2.5d |
| ENH-11 | 定时调度器 | 每日自动下载+开机自启 | S7a（S6后） | 1.5d |

实施策略：推荐 ENH-10 前移到 S3a，ENH-11 保持 S6 后实施（S7a）
...
- 圈位：`S3a`（紧接 S2b PASS 后，进入 S3 前）
- 主目标：交付 ENH-10 最小生产可用能力，不扩展调度器能力
- run：
...
#### 方案 A：S3a + S7a 分层实施（推荐）

```
S0 → S1 → S2 → S3a(ENH-10) → S3 → S4 → S5 → S6 → S7a(ENH-11)
              (2.5d)                                     (1.5d)
```

**优点**：
- 在回测圈（S3）前补齐历史数据拉取能力
- 不把 ENH-11 提前引入，避免运营能力打断主线
- 与“先数据、后调度”的依赖顺序一致

**缺点**：
- 需要新增一个微圈（S3a）做显式治理记录
...
**缺点**：
- S3 前缺少自动分批/断点能力，回测准备效率较低
- 把关键数据能力放到过晚阶段

#### 方案 C：S0 扩展实施（激进）

```
S0 (4.5d → 7.0d, 含 ENH-10) → S1 → S2 → S3 → S4 → S5 → S6 → S7a(ENH-11)
```
...
在 S2 与 S3 之间新增 S3a，在 S6 后新增 S7a：

```markdown
| S3a | 🔄 采集增强 | 分批+断点续传+多线程 | - | 2.5d | 📋 未开始 |
| S7a | ⏰ 自动调度 | 每日自动下载+开机自启 | - | 1.5d | 📋 未开始 |
| **合计** | | | | **34.1d** | |
```
...
```markdown
| ENH-10 | 数据采集增强 | S3a（S2后、S3前） | — | 高：历史数据拉取效率 |
| ENH-11 | 定时调度器 | S7a（S6后） | — | 中：日常运营便利性 |
```
...
### 6.1 推荐方案：S3a + S7a 分层实施

**理由**：
1. ENH-10 对 S3 数据准备直接增益，应前移
2. ENH-11 属于运营自动化，应后置
3. 保持主线“信号与验证闭环优先”的节奏
4. 降低单圈复杂度，符合 6A 的单目标收口原则

**时间线**：
```
Day 0-17.4:  S0-S2 完成
Day 18-20.9: S3a 实施 ENH-10
Day 21-30.1: S3-S6 完成
Day 31-32.6: S7a 实施 ENH-11
```

**S3a 前兜底方案（若 ENH-10 延期）**：
...
### 6.2 备选方案：S6 后打包实施（保守）

**理由**：
- 若近期目标只做主线闭环，可将两项外挂统一后置

**时间线**：
```
Day 0-30.1:  S0-S6 完成
Day 31-33.6: ENH-10 实施
Day 34-35.1: ENH-11 实施
```

### 6.3 激进方案：S0 扩展 ENH-10（不推荐）

**理由**：
- 仅在你要求“最早拿到 ENH-10 且能接受 S0 延长”时考虑

**时间线**：
```
Day 0-7.0:   S0(含 ENH-10) 完成
Day 7-30.1:  S1-S6 完成
Day 31-32.6: S7a(ENH-11) 实施
```
...
1. 完成 S0 基础版（4.5d）
   ↓
2. 完成 S1-S2 主线（到可推荐闭环）
   ↓
3. 执行 S3a：实施 ENH-10（2.5d）
   ↓
4. 完成 S3-S6 主线
   ↓
5. 执行 S7a：实施 ENH-11（1.5d）
   ↓
6. 全自动数据采集系统完成（总计约 32.6d）
```

### 9.3 关键决策点

**决策 1：何时实施 ENH-10/11？**
- 推荐：ENH-10 在 S3a，ENH-11 在 S7a
- 保守：两项都 S6 后
- 激进：ENH-10 并入 S0

**决策 2：是否必须实施？**
- ENH-10：对 S3 数据准备强烈建议实施
- ENH-11：非必须，但能显著提升日常运营体验

**决策 3：实施优先级？**
- 高优先级：S0-S2 主线 + ENH-10（S3a）
- 中优先级：S3-S6 主线
- 低优先级：ENH-11（S7a）

### 9.4 落地结果（2026-02-13）

1. `ENH-10` 已纳入 `S3a` 扩展微圈。
2. `ENH-11` 已纳入 `S7a` 扩展微圈。
3. 对应执行入口已更新：
   - `docs/design/enhancements/eq-improvement-plan-core-frozen.md`
   - `Governance/Capability/SPIRAL-CP-OVERVIEW.md`
   - `Governance/SpiralRoadmap/SPIRAL-PRODUCTION-ROUTES.md`
4. 本文档转为提案归档记录，不再作为唯一执行入口。
